...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var primColorLbl: UILabel!
    @IBOutlet weak var detColorLbl: UILabel!
    @IBOutlet weak var secColorLbl: UILabel!
    ...
    @IBAction func prevBtnPressed(_ sender: Any) {
        magic(albumArt: realImages[scrollView.currentPage-1], scale: 11)
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        for i in 0...17{
            realImages.append(UIImage(named: "\(i).jpg")!)
            let imView = UIImageView(image: realImages[i])
            images.append(imView)
            
            let X: CGFloat = 20 + scrollView.frame.size.width * CGFloat(i)
            contentWidth += X
            scrollView.addSubview(imView)
            imView.frame = CGRect(x: X, y: 10, width: 280, height: 280)
        }
        magic(albumArt: realImages[0],scale: scale)
    ...
    func magic(albumArt: UIImage, scale: Int){
        let colors = albumArt.getColors(scaleDownSize: CGSize(width: scale, height: scale))
        //    let usedImage = #imageLiteral(resourceName: "black").withRenderingMode(Template)
        //  playBtn.image = usedImage
        primColorLbl.textColor = colors.primaryColor
        detColorLbl.textColor = colors.detailColor
        secColorLbl.textColor = colors.secondaryColor
        view.backgroundColor = colors.backgroundColor
        prevBtn.tintColor = colors.secondaryColor
    }
}*/
